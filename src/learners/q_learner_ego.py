...
class QLearnerEgo:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer is not None:
            if args.mixer == "vdn":
                ...
                self.mixer = QMixer(args, self.args.n_ego)
            else:
        ...
        if self.args.standardise_returns:
            self.ret_ms = RunningMeanStd(shape=(self.n_ego,), device=device)
        if self.args.standardise_rewards:
            self.rew_ms = RunningMeanStd(shape=(1,), device=device)

        self.prev_feature_layer = {}

    def cache_feature_layer(self, head_id):
        self.prev_feature_layer = {} # do not store the prev prev params
        self.prev_feature_layer[head_id] = {n: p.detach().clone() for n, p in self.mac.agent.feature_layer.named_parameters()}

    def reg_penalty(self):
        loss = {}
        for head_id in self.prev_feature_layer.keys():
            loss[head_id] = 0
            target_param = self.prev_feature_layer[head_id]
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        reg_loss = th.tensor(0)
        if self.args.reg_coef > 0 and len(self.prev_feature_layer) > 0:
            reg_losses = self.reg_penalty()
            reg_loss = sum([loss for _, loss in reg_losses.items()])
        ...
        # Mix
        if self.mixer is not None and self.args.n_ego > 1:
            chosen_action_qvals = self.mixer(chosen_action_qvals[:, :, :self.args.n_ego], batch["state"][:, :-1])
            target_max_qvals = self.target_mixer(target_max_qvals[:, :, :self.args.n_ego], batch["state"][:, 1:])
        if self.args.standardise_returns:
            target_max_qvals[:, :, :self.args.n_ego] = target_max_qvals[:, :, :self.args.n_ego] * th.sqrt(self.ret_ms.var) + self.ret_ms.mean
        ...
        loss = td_loss + self.args.reg_coef * reg_loss
