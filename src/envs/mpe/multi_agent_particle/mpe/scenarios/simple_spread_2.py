...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, agent in enumerate(world.agents):
            if i == 0:
                agent.state.p_pos = np.array([np.random.uniform(-0.5 - EPS, -0.5 + EPS), np.random.uniform(0 - EPS, 0 + EPS)])
            elif i == 1:
                agent.state.p_pos = np.array([np.random.uniform(0.5 - EPS, 0.5 + EPS), np.random.uniform(0 - EPS, 0 + EPS)])
        ...
        for i, landmark in enumerate(world.landmarks):
            if i == 0:
                landmark.state.p_pos = np.array([np.random.uniform(0 - EPS, 0 + EPS), np.random.uniform(1 - EPS, 1 + EPS)])
            elif i == 1:
                landmark.state.p_pos = np.array([np.random.uniform(0 - EPS, 0 + EPS), np.random.uniform(-1 - EPS, -1 + EPS)])
    ...
    def is_close(self, entity1, entity2):
        ...
        return True if dist < dist_min * 1.4 else False
